...
| 2026-06-24 | 60% fully covered | +ADR-0023 Dialogue 三层数据流 (.dlg → YAML → Foundation DialogueRuntime) + Scene-Agnostic Provider；首个 dialogue 系统正式 ADR，收口 `production/epics/dialogue-system/` 早期设计意图；状态 Accepted。chapter_00 4 段对话端到端跑通（commit 255a510 / 6d53546 / 2a4618b / 1129350），`tools/dialogue_compiler.py` 作者工具落（commit 3b1dfd2 / 48a1ce7）。Cross-ADR Conventions 不新增 — 仅消费 ADR-0002 (CanvasLayer 40)、Foundation 既有 `IDialogueConditionValueProvider` / `IDialogueEvent`。 |
| 2026-06-24 | 60% fully covered | +ADR-0024 iso Tileset 资产层暂撤 ≠ 方向否决：commit 24c34ba 删除 jiangnan-iso-v1 全部资产 (8 文件 + showcase + 测试场景 + scene_markup_tool 插件)，因 image_gen 出图质量不满意。**ADR-0022 方向继续生效**（IsoProjection + Iso4 角色动画 + battle scene iso 蒙皮 全部不动）。过渡期使用 `feng-zhi/assets/maps/jiangnan_riverside/jiangnan_ground_tiles.tres`（commit fbbc5be 非-iso 平面）作 chapter_00 explore 占位，**不进 art-bible**。6 个 S8-tileset-* backlog 全部 `depends-on S8-tileset-iso-redesign`（sprint-status.yaml commit 75883c6）；何时启动 redesign 由 owner 按信号 A/B/C 拍板。状态 Accepted。 |
