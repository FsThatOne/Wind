...
Resolve skills by logical skill name through the current host's skill mechanism. If the host exposes a skill as an invocable tool, invoke it by name. If the host exposes skill files instead, read the host-resolved `SKILL.md` and execute its workflow.

If host lookup cannot resolve a skill by name, fall back to `<skillName>/SKILL.md` under the project-root `.claude/skills`, then `.cursor/skills`.

If a required skill cannot be resolved by host lookup or fallback paths, **STOP** and tell the user to sync or install that skill for the current AI IDE/agent host.
...
This stage skill uses shared assets from the `adk-sdt` skill package.

Set `SDT_ASSET_ROOT` to the host-resolved `resources` directory of the installed `adk-sdt` skill.

Use assets only via `SDT_ASSET_ROOT`, for example:

- Check script: `SDT_ASSET_ROOT/scripts/check-prerequisites.js`
- Skill profiles: `SDT_ASSET_ROOT/templates/sdt-skill-profiles.json`
- Backend task template: `SDT_ASSET_ROOT/templates/sdt-backend-task-template.md`
- Frontend task template: `SDT_ASSET_ROOT/templates/sdt-frontend-task-template.md`
...
- Read SDT skill profiles from:
  - `SDT_ASSET_ROOT/templates/sdt-skill-profiles.json`
- The file defines:
  - `domains`: domain → `profile`
  - `profiles`: profile → `{ backend: {LOGICAL_KEY: skillName}, frontend: {...} }`
  - `TEXT_CASE_GENERATE`: platform-specific prd2case-style skill used to generate `test/case.md`
  - `defaultProfile`
...
2. Run `node "$SDT_ASSET_ROOT/scripts/check-prerequisites.js" --paths-only --json` from repo root.
...
3. If multiple features exist, use the host-compatible structured question tool per `User Interaction (host compatibility)` to select the target feature.
...
2. **OPTIONAL**: Read `FEATURE_DIR/plan.md` if exists (for technical context).
3. **OPTIONAL**: Read `FEATURE_DIR/data-model.md` if exists (for entity/field reference).
...
1. Read `SDT_ASSET_ROOT/templates/sdt-skill-profiles.json`.
2. Infer `DOMAIN` per **Domain inference**.
3. Resolve `PROFILE` and the platform mapping per **Profile resolution (with fallback)**.
   - Backend case generation requires `SKILLS.TEXT_CASE_GENERATE`.
   - Frontend case generation requires `SKILLS.TEXT_CASE_GENERATE`.
...
1. Generate test cases according to the **inferred** target platform from Step 4. Use `FEATURE_DIR/spec.md` as the primary input.
    - If target platform is **Backend**:
        1. Ask the user for test granularity using `AskUserQuestion`:
        ...
        2. Run the workflow resolved as `SKILLS.TEXT_CASE_GENERATE` per **Skills (host compatibility)** (invoke by name in Claude Code when available **or** resolve `SKILL.md` for `SKILLS.TEXT_CASE_GENERATE` and follow it) to generate backend API test cases. **Pass `PRIORITY` to the workflow** and instruct it to only emit cases whose priority falls within the selected set; cases outside the selected priorities MUST be excluded from the output.
    - If target platform is **Frontend**, run the workflow resolved as `SKILLS.TEXT_CASE_GENERATE` per **Skills (host compatibility)** (invoke by name in Claude Code when available **or** resolve `SKILL.md` for `SKILLS.TEXT_CASE_GENERATE` and follow it), execute task type 2 to generate web e2e test cases.
...
1. Based on the target platform from Step 4, if it is **Backend**, read the template:
   `SDT_ASSET_ROOT/templates/sdt-backend-task-template.md`.
   If it is **Frontend**, read the template: `SDT_ASSET_ROOT/templates/sdt-frontend-task-template.md`.
...
2. If any are missing:
   - Use the host-compatible structured question tool per `User Interaction (host compatibility)` when the information can be collected through predefined options.
...
4. **Skip this when the target platform is Frontend**: Check whether test data (Test Data) exists in `FEATURE_DIR/test/case.md`. If it does not exist, prompt the user to input test data, then populate it into `FEATURE_DIR/test/case.md`. case.md must follow the language configuration in Language Setting.
...
| Skill `SKILL.md` missing          | Tell user to ensure the required skill is installed for the current AI IDE/agent host                                |
