...
When resolving any SDT/helper skill by name, use the host skill mechanism first; if missing, read `<skillName>/SKILL.md` from the project-root `.claude/skills`, then `.cursor/skills`.
...
- Report template: `SDT_ASSET_ROOT/templates/sdt-report-template.md`
...
2. Run `node "$SDT_ASSET_ROOT/scripts/check-prerequisites.js" --json --require-tasks --include-tasks` from repo root and parse FEATURE_DIR and AVAILABLE_DOCS list. All paths must be absolute.
...
1. Read SDT skill profiles from `SDT_ASSET_ROOT/templates/sdt-skill-profiles.json`.
...
- **Follow task-specific workflow details from `test/task.md`**: Domain-specific steps such as Backend deployment cluster selection, deployment fan-out, and polling semantics are defined in the generated task file and must be executed as written there.
...
### 4: Summary:
1. Read the report template: `SDT_ASSET_ROOT/templates/sdt-report-template.md`.
