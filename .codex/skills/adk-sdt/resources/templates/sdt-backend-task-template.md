...
> 若上表中仍出现 `{{...}}` 形式的占位符（未被替换），说明 SDT 注入失败。请停止执行，并重新运行 `/adk-sdt-ff`，或检查 `adk-sdt` Skill 包内的 `resources/templates/sdt-skill-profiles.json`。
...
| :--- | :--- |
| 状态 | `[ ]` |
| 执行方式 | **必须使用 `AskUserQuestion` 工具**向用户确认部署检查、部署执行；若用户选择需要部署，再优先基于 PSM 部署信息确认目标部署目标 |
...
3. **当用户选择需要部署时，必须先根据 PSM 推断部署区域，再确认目标部署目标**：
   - 如果 Input 中已有 PSM List，先对每个 PSM 逐个运行 `gdpa-cli run psm-info -i '{"psm":"<PSM>"}'` 获取部署区域信息。`psm-info` 通常返回粗粒度 `region` / `regions`，例如 `i18n`、`cn`、`boe`，不一定直接返回标准 VRegion/VDC/IDC。
   - 多个 PSM 时，先对各 PSM 的 `regions` key 取交集；如果交集只有一个粗粒度区域，则选中该区域。如果某个 PSM 只有一个 `region`，也可作为该 PSM 的默认候选。
   - 如果得到多个粗粒度区域，优先根据 `psm-info` 返回的服务部署信息帮用户选择目标区域：优先选择覆盖所有待部署 PSM、与 `{{ENV_TYPE}}` / `{{ENV_NAME}}` 匹配、且存在可用实例或明确可部署位置的区域；若仍无法唯一判断，再用 `AskUserQuestion` 基于 `psm-info` 返回的候选项向用户确认，不要仅展示静态全集。
   - 将粗粒度区域映射到 `env` skill 支持的标准 VRegion：`i18n` 可对应 `Singapore-Central`，`cn` 可对应 `China-North` / `China-East`，`boe` 可对应 `China-BOE`, `boei18n` 可对应 `US-BOE`，`euttp` 可对应 `EU-TTP` / `EU-TTP2` / `US-EastRed`，`usttp` 可对应 `US-TTP` / `US-TTP2`。如果 `{{V_REGION}}` 已给出且属于所选粗粒度区域，直接使用 `{{V_REGION}}` 构造 `DEPLOY_TARGETS`。
   - 如果粗粒度区域无法唯一映射到标准 VRegion，且 `{{V_REGION}}` 也不可用或不匹配，再提问：`请选择要同时部署到的目标 VRegion（可多选）：`；选项应优先限制在所选粗粒度区域对应的候选 VRegion 内。若还需要 VDC/IDC，再继续询问或通过 `env` skill 的 `create_suggest` 获取可用 `idc` / `zone` / `virtual_cluster` 后让用户确认。
   - 将最终结果转换为结构化 `DEPLOY_TARGETS`，每项至少包含 `label`、`vregion`、`idc`，如有明确虚拟集群则补充 `cluster` / `virtual_cluster`。
...
- {{V_REGION}}: 测试任务执行时部署的标准 VRegion，必须与 `env` skill 支持值一致。可选项以 `env` skill 的「vregion 可选值」为准：`Singapore-Central`、`US-East`、`US-West`、`China-North`、`China-East`、`EU-TTP`、`EU-TTP2`、`US-TTP`、`US-TTP2`、`US-EastRed`、`China-BOE`、`US-BOE`。`env` skill 要求显式传入 `vregion`，未查询到时必须用 AskUserQuestion 询问
