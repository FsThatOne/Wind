...
6. **Synchronously update all existing design-stage documents:**
    ...
    **If `tasks.md` exists:** ← **THIS IS MANDATORY IF tasks.md EXISTS**
    ...
    - Treat `- [x]` tasks as implementation history; modify or reopen them only when the three-mode rule below explicitly permits.
    ...
    - **Three-mode update rule for tasks.md:**
      * **Modify**: If a clarification affects an incomplete task (`- [ ]`), edit its title / description / acceptance criteria in place.
      * **Reopen**: If the clarification's new requirement is incompatible with the original implementation of a completed task (`- [x]`), reset that task back to `- [ ]`, edit its body, and append a single annotation line `Reopened by clarify: <one-line reason>` (reason only — date is intentionally omitted to preserve prompt-cache stability).
      * **Append**: If the clarification introduces a new deliverable, OR adds new work to a completed task whose original implementation is still valid (in which case keep the `- [x]`), add a new `- [ ]` task.
...
7. Validation (performed after EACH write plus final pass):
    ...
    **Cross-document consistency checks (REQUIRED):**
    ...
    - **For affected tasks, validation expects the three-mode rule from Step 6.C to have been applied (Modify / Reopen / Append). Pure rewriting of a completed task while leaving it checked is not allowed.**
    - **FAIL validation if:**
...
9. Report completion (after questioning loop ends or early termination):
    ...
    **Document Update Summary (REQUIRED for each document in DISCOVERED_DOCS):**
    ...
    | tasks.md | ✓/✗    | ✓/✗     | (Modified/Reopened/Added IDs) | (reason or "-")   |
...
- **When updating tasks.md, apply the three-mode update rule (Modify / Reopen / Append) defined in Step 6.C. The "Sections Changed" entry for tasks.md in the Document Update Summary MUST list task IDs grouped as `Modified: ... / Reopened: ... / Added: ...`.**
