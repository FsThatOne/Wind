...
/// 与 yaml 数据出现偏差时优先以本文件为准（演示范围内的固化 fixture）。
/// </summary>
public static class CombatDemoBandit1v1Fixture
{
    ...
    //   - 4 个 demo_* 招式仅作 UI 呈现，提交时由 TacticsBattleGame 翻译回 luo_han_quan/tie_bi_heng_lan
    ...
    //   - EnemyInitialStaggerOverride = 5 由 TacticsBattleGame._Ready 在 facade.InitiateBattle 之后调
    //     bandit.AddStagger(5) 直接预置
    //   - 演出触发链路: 玩家选决胜行 → BattleAction.ActionType=Decisive → ResolutionService.ExecuteDecisive
    //     → DamageDealtEvent(VisualRelation=Decisive) → TacticsBattleGame.OnDamageDealtForDecisive
